...
import copy
import json
from tzm.tzdb import mdb, DbDataCo
...
flex_bubble_template = {
    ...
    "hero": {
        ...
        "aspectRatio": "20:13",
        "aspectMode": "cover",
        "action": {"type": "uri", "uri": "http://www.tarozafra.com/"},
    },
    "body": {
        "type": "box",
        "layout": "vertical",
        "contents": [
            {"type": "text", "text": "Santiago Cake", "weight": "bold", "size": "xl"},
            {
                "type": "box",
                "layout": "vertical",
                "margin": "lg",
...
# Make menu with flexmessage


def make_menu():
    flex_message_json = {"type": "carousel", "contents": []}
    items = [
        {
            "name": "Santiago Cake",
    ...
    for item in items:
        ...
        flex_bubble["body"]["contents"][1]["contents"][1]["contents"][1][
            "text"
        ] = "$" + str(item["price"])
        flex_bubble["body"]["contents"][1]["contents"][0]["contents"][1]["text"] = item[
            "Ingredients"
        ]
    ...
    if event.message.type == "text":
        if event.message.text == "I would like to have Santiago cake. ":
            line_bot_api.reply_message(
                event.reply_token, TextSendMessage(text="您要幾片(請只回答數字)?")
            )
        else:
            flex_message = FlexSendMessage(alt_text="hello", contents=flex_message_json)
